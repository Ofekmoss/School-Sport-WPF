	...
	public class MatchChampionshipEditorView : ChampionshipEditorView
	{
		...
		private void tbBuildMatches_Click(object sender, System.EventArgs e)
		{
			...
			if (group != null)
			{
				...
				foreach (Sport.Data.Entity board in boards)
				{
					Sport.Common.RangeArray ra = Sport.Common.RangeArray.Parse(rangeField.GetText(board));
					if (ra[group.Teams.Count])
						al.Add(board);
				}
				...
				Sport.UI.Dialogs.GenericEditDialog ged = 
					new Sport.UI.Dialogs.GenericEditDialog("בניה מתבנית");

				ged.Items.Add("לוח משחקים:", Sport.UI.Controls.GenericItemType.Selection, null, (object[]) al.ToArray(typeof(Sport.Data.Entity)));
		...
		{
			get { return _round == -1 ? null : Group.Rounds[_round]; }
		...
		{
			get { return _cycle == -1 ? null : Round.Cycles[_cycle]; }
		...
		{
			get { return _tournament == -1 ? null : Cycle.Tournaments[_tournament]; }
		...
		{
			get { return _match == -1 ? null : Cycle.Matches[_match]; }
		...
		protected override Sport.Documents.Document CreateGroupDocument(System.Drawing.Printing.PrinterSettings settings)
		{
			...
			int selCycle=-1;
			if (this.Phase != null)
				selPhase = this.Phase.Index;
			if (this.Group != null)
				selGroup = this.Group.Index;
			if (this.Round != null)
				selRound = this.Round.Index;
			if (this.Cycle != null)
				selCycle = this.Cycle.Index;
			...
			//matchesSectionObject.Championship = Championship;
			//matchesSectionObject.Fields = matchesGridPanel.Columns;
		...
		protected override Sport.Documents.Document CreateRankingDocument(System.Drawing.Printing.PrinterSettings settings)
		{
			...
			RankingSectionObject rankingSectionObject = new RankingSectionObject();
		...
		private void tbReferreReport_Click(object sender, EventArgs e)
		{
			...
			Sport.Championships.Tournament tournament=this.Tournament;
			Sport.Championships.Cycle cycle=this.Cycle;
			if (tournament != null)
			{
				int index=tournament.Index;
				foreach (Sport.Championships.Match match in tournament.Cycle.Matches)
				{
					if (match.Tournament == index)
						matches.Add(match);
				}
			}
			else if (cycle != null)
			{
				matches.AddRange(cycle.Matches);
			}
			
			string strCaption=(tournament != null)?"טורניר":"מחזור";
			if (matches.Count == 0)
			{
				Sport.UI.MessageBox.Error("אין משחקים ב"+strCaption+" זה", 
					"בניית אליפות");
				return;
			}
			...
			foreach (Sport.Championships.Match match in matches)
				totalRefCount += match.RefereeCount;
			if (totalRefCount == 0)
			{
				Sport.UI.MessageBox.Error("ב"+strCaption+" זה אין הזמנות שופטים", 
					"בניית אליפות");
				return;
			}
			
			DateTime matchesDate=DateTime.MinValue;
			bool blnDifferentDates=false;
			foreach (Sport.Championships.Match match in matches)
			{
				DateTime curDate=match.Time;
				if ((matchesDate.Year < 1900)&&(curDate.Year > 1900))
					matchesDate = curDate;
				if ((matchesDate.Year > 1900)&&(curDate.Year > 1900)&&
					(!Sport.Common.Tools.IsSameDate(matchesDate, curDate)))
				{
					blnDifferentDates = true;
					break;
				}
			}
			if (matchesDate.Year < 1900)
			{
				Sport.UI.MessageBox.Error("לא מוגדרים תאריכים עבור משחקי ה"+strCaption, 
					"בניית אליפות");
				return;
			}
			if (blnDifferentDates)
			{
				Sport.UI.MessageBox.Error("קיימים תאריכים שונים במשחקי ה"+strCaption, 
					"בניית אליפות");
				return;
			}
			
			Sport.Entities.Facility matchesFacility=null;
			bool blnDifferentFacilities=false;
			foreach (Sport.Championships.Match match in matches)
			{
				Sport.Entities.Facility curFacility=match.Facility;
				if ((matchesFacility == null)&&(curFacility != null))
					matchesFacility = curFacility;
				if ((matchesFacility != null)&&(curFacility != null)&&
					(matchesFacility.Id != curFacility.Id))
				{
					blnDifferentFacilities = true;
					break;
				}
			}
			
			if (matchesFacility == null)
			{
				Sport.UI.MessageBox.Error("לא מוגדר מתקן עבור משחקי ה"+strCaption, 
					"בניית אליפות");
				return;
			}
			if (blnDifferentFacilities)
			{
				Sport.UI.MessageBox.Error("מוגדרים מתקנים שונים עבור משחקי ה"+strCaption, 
					"בניית אליפות");
				return;
			}
			...
			Documents.ChampionshipDocuments champDoc=
				...
				Documents.ChampionshipDocumentType.RefereesReport, 
				(Sport.Championships.Match[]) 
				matches.ToArray(typeof(Sport.Championships.Match)));
			champDoc.Print();
		}
	}
}
