	...
	public class ChampionshipEditorView : Sport.UI.View
	{
		...
		private void tbTeamUp_Click(object sender, System.EventArgs e)
		{
			...
			if (teamA != null && teamA.Position > 0)
			{
				...
				if (teamB != null)
				{
					if (group.ReplaceTeams(teamA.Index, teamB.Index))
					{
		...
		private void tbDivideTeams_Click(object sender, System.EventArgs e)
		{
			if (Phase != null)
			{
				TeamsDivisionForm tdf = new TeamsDivisionForm(championship, Phase.Index);
		...
		protected virtual void OnPhaseChange()
		{
			...
			tbPhaseName.Text = _phase == -1 ? null : Phase.Name;
			tbRemovePhase.Enabled = _phase != -1;
			tbPhaseDefinition.Enabled = _phase != -1;
			tbPhaseName.Enabled = _phase != -1;
			tbMovePhase.Enabled = _phase != -1 && championship.Phases.Count > 1;
			selecting = s;
		}
		...
		protected virtual void OnTeamChange()
		{
			...
			tbMoveTeam.Enabled = _team != -1 && Group.Teams.Count > 1;
			selecting = s;
		}
		#endregion
		...
		public override void Open()
		{
			...
			if (ccid != null)
			{
				...
				championship = Sport.Championships.Championship.GetChampionship(champId);
		...
		private void tbPrint_Click(object sender, System.EventArgs e)
		{
			...
			if (settingsForm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
			{
				...
				else if (panelRankingView.Visible)
		...
		private void SaveStructure()
		{
			...
			try
			{
				saveResult = (championship.Save().Length == 0);
			}
			catch (Exception e)
			{
				Sport.UI.MessageBox.Show(e.Message, "שגיאה", System.Windows.Forms.MessageBoxIcon.Error);
				Sport.UI.MessageBox.Show("שגיאה בעת שמירת מבנה אליפות, לא ניתן לשמור", "שגיאה", System.Windows.Forms.MessageBoxIcon.Warning);
			}
			Sport.UI.Dialogs.WaitForm.HideWait();
		...
		protected virtual int ImportResults()
		{
			...
			{
				competition.SetResults(-1, (Sport.Championships.CompetitorResult[])
