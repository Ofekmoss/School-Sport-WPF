...
namespace Sportsman.Producer
{
	public class OpenChampionshipCommand : Sport.UI.Command
	{
		...
		private static GenericItem _regionItem = null;
		private static GenericItem _sportItem = null;
		...
		static OpenChampionshipCommand()
		{
			...
			_regionItem = new GenericItem("מחוז", GenericItemType.Selection, selRegion, regions);
			_sportItem = new GenericItem("ענף ספורט", GenericItemType.Selection, null, sports);
			_dialog.Items.Add(_regionItem);
			_dialog.Items.Add(_sportItem);
			_dialog.Items.Add(new GenericItem("אליפות", GenericItemType.Selection));
			_dialog.Items.Add(new GenericItem("קטגוריה", GenericItemType.Selection));
			_dialog.Confirmable = false;

			_regionItem.ValueChanged += new EventHandler(RegionChanged);
			_sportItem.ValueChanged += new EventHandler(SportChanged);
			...
			_regionItem.Nullable = false;
			_sportItem.Nullable = false;
			_dialog.Items[2].Nullable = false;
			_dialog.Items[3].Nullable = false;

			RegionChanged(null, EventArgs.Empty);
		}
		...
		public override void Execute(string param)
		{
			...
			else
			{
				...
				if (category != null)
				{
					...
					try
					{
						championship = Sport.Championships.Championship.GetChampionship(category.Id, true);
					}
					catch (Exception e)
					{
		...
		private static void RegionChanged(object sender, EventArgs e)
		{
			if (_sportItem != null)
			{
				Sport.Data.EntityFilter entitityFilter = null;
				if (_sportItem != null && _regionItem.Value != null)
				{
					int selectedRegion = ((Sport.Data.Entity)_regionItem.Value).Id;
					if (selectedRegion != Sport.Entities.Region.CentralRegion)
					{
						entitityFilter = new Sport.Data.EntityFilter(
							new Sport.Data.EntityFilterField((int)Sport.Entities.Sport.Fields.CentralRegionOnly, 1, true));
					}
				}
				try
				{
					_sportItem.Values = Sport.Entities.Sport.Type.GetEntities(entitityFilter);
				}
				catch
				{
				}
