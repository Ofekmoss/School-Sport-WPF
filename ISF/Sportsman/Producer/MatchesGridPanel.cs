...
namespace Sportsman.Producer
{
		...
		Sport.UI.Controls.IGridSource
	{
		...
		public MatchesGridPanel()
		{
			...
			_grid.SelectionMode = System.Windows.Forms.SelectionMode.MultiExtended;
			_grid.Source = this;
		...
		public Sport.UI.Controls.Grid.GridSelection GetGridSelection()
		{
			return _grid.Selection;
		}
		...
		{
			if (e.Button != System.Windows.Forms.MouseButtons.Right || _phase == null || _phase.Championship.Editing)
				return;

			List<KeyValuePair<string, EventHandler>> items = new List<KeyValuePair<string, EventHandler>>();
			if (_selectedRow is Sport.Championships.Cycle || _selectedRow is Sport.Championships.Round || _grid.Selection.Rows.Length > 1)
			{
				items.Add(new KeyValuePair<string, EventHandler>("הגדרת מועדים, מתקנים, ובעלי תפקידים", new EventHandler(TimeFacilityClick)));
				items.Add(new KeyValuePair<string, EventHandler>("הסרת מועדים גורפת", new EventHandler(ClearAllTimes)));
				items.Add(new KeyValuePair<string, EventHandler>("הסרת בעלי תפקידים", new EventHandler(ClearAllFunctionaries)));
			}
			else if (_selectedRow is Sport.Championships.Match)
			{
				items.Add(new KeyValuePair<string, EventHandler>("פתח", new EventHandler(GridDoubleClick)));
			}
			if (items.Count > 0)
			{
				System.Windows.Forms.ContextMenu menu = new System.Windows.Forms.ContextMenu(items.ConvertAll(item =>
					new System.Windows.Forms.MenuItem(item.Key, item.Value)).ToArray());
		...
		{
			if (_phase == null || _phase.Championship == null)
				return;
		...
		private List<Sport.Championships.Match> GetSelectedMatches(ref string owner)
		{
			List<Sport.Championships.Match> matches = new List<Sport.Championships.Match>();
			if (_grid.Selection.Rows.Length > 1)
				return _grid.Selection.Rows.ToList().ConvertAll(rowIndex => (Sport.Championships.Match)_rows[rowIndex]).FindAll(m => m != null);

			List<Sport.Championships.Cycle> cycles = new List<Sport.Championships.Cycle>();
			...
			bool isTournament = _selectedRow is Sport.Championships.Tournament;
			return cycles.ConvertAll(c => c.Matches.OfType<Sport.Championships.Match>().ToList().FindAll(m =>
				!isTournament || m.Tournament == _tournament.Index)).AggregateAll<Sport.Championships.Match>();
		}

		private List<Sport.Championships.Match> GetSelectedMatches()
		{
			string dummy = "";
			return GetSelectedMatches(ref dummy);
		}

		private void SetMatchesTimeFacility()
		{
			string owner = "";
			List<Sport.Championships.Match> matches = GetSelectedMatches(ref owner);
			if (matches.Count > 0)
			{
				matches.Sort((m1, m2) =>
				{
					...
					{
						int d = m1.Tournament.CompareTo(m2.Tournament);
						return (d == 0) ? m1.Index.CompareTo(m2.Index) : d;
					}

					if (m1.Cycle.Round == m2.Cycle.Round)
						return m1.Cycle.Index.CompareTo(m2.Cycle.Index);
					
					if (m1.Cycle.Round.Group == m2.Cycle.Round.Group)
						return m1.Cycle.Round.Index.CompareTo(m2.Cycle.Round.Index);

					Sport.Championships.Group g1 = m1.Cycle.Round.Group;
					Sport.Championships.Group g2 = m2.Cycle.Round.Group;
					if (g1.Phase == g2.Phase)
						 return g1.Index.CompareTo(g2.Index);
					
					if (g1.Phase.Championship == g2.Phase.Championship)
						return g1.Phase.Index.CompareTo(g2.Phase.Index);

					return g1.Phase.Championship.Name.CompareTo(g2.Phase.Championship.Name);
				});

				if (matchesTimeFacilityForm == null)
					matchesTimeFacilityForm = new MatchesTimeFacilityForm();

				matchesTimeFacilityForm.SetMatches(owner, matches.ToArray());
		...
		private void SetGlobalValues(string title, DateTime? date, int[] functionaries)
		{
			List<Sport.Championships.Match> matches = GetSelectedMatches();
			if (matches.Count > 0)
			{
				string caption = "";
				if (_selectedRow is Sport.Championships.Cycle)
					caption = "במחזור זה";
				else if (_selectedRow is Sport.Championships.Round)
					caption = "בסיבוב זה";
				else if (_selectedRow is Sport.Championships.Tournament)
					caption = "בטורניר זה";
				else
					caption = "הנבחרים";
				if (Sport.UI.MessageBox.Ask("האם להסיר " + title + " של כל המשחקים " + caption + "?", false))
				{
					matches.ForEach(match => match.Set(match.Number, date == null  ? match.Time : date.Value, 
						match.Facility, match.Court, functionaries == null ? match.Functionaries : functionaries));
					Rebuild();
				}
			}
		}

		private void ClearAllTimes(object sender, EventArgs e)
		{
			SetGlobalValues("תאריכים", DateTime.MinValue, null);
		}

		private void ClearAllFunctionaries(object sender, EventArgs e)
		{
			SetGlobalValues("בעלי תפקידים", null, new int[] {});
		}
	}
}
