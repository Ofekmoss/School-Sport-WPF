...
namespace Sportsman.Producer
{
		...
		Sport.UI.Controls.IGridSource
	{
		...
		private void ResetRankingTable()
		{
			...
			int curFieldIndex = 0;
			_grid.Groups[1].Columns.Add(curFieldIndex++, "מיקום", 20, System.Drawing.StringAlignment.Center);
			_grid.Groups[1].Columns.Add(curFieldIndex++, "קבוצה", 100);
			...
			if (_championship != null)
			{
				...
				if (_rankingTable != null)
				{
					...
					{
						_grid.Groups[1].Columns.Add(curFieldIndex++, _rankingTable.Fields[n].Title, 20, System.Drawing.StringAlignment.Center);
					}
					if (this.Championship is Sport.Championships.CompetitionChampionship)
					{
						_grid.Groups[1].Columns.Add(curFieldIndex++, "עמידה בתקנון", 25, System.Drawing.StringAlignment.Center);
					}
				}
			}

			this.fieldCount = curFieldIndex;
		}
		#endregion
		...
		private static Sport.UI.Controls.Style invalidCellStyle;
		private static Sport.UI.Controls.Style selectedGroupStyle;
		static RankingGridPanel()
		{
			groupStyle = new Sport.UI.Controls.Style();
			groupStyle.Background = new System.Drawing.SolidBrush(System.Drawing.Color.FromArgb(50, 150, 196));
			groupStyle.Foreground = System.Drawing.Brushes.White;
			
			selectedGroupStyle = new Sport.UI.Controls.Style();
			selectedGroupStyle.Background = new System.Drawing.SolidBrush(System.Drawing.Color.FromArgb(25, 120, 146));
			selectedGroupStyle.Foreground = System.Drawing.Brushes.White;

			invalidCellStyle = new Sport.UI.Controls.Style();
			invalidCellStyle.Background = new System.Drawing.SolidBrush(System.Drawing.Color.FromArgb(202, 202, 202));
			invalidCellStyle.Foreground = System.Drawing.Brushes.Black;
		}
		...
		public string GetText(int row, int field)
		{
			...
			if (this.Championship is Sport.Championships.CompetitionChampionship)
			{
				if (_rankingTable != null)
				{
					int lastField = this.GetFieldCount(row) - 1;
					if (field == lastField)
					{
						bool blnValid = true;
						for (int n = 2; n < _rankingTable.Fields.Count; n++)
						{
							string strTitle = _grid.Groups[1].Columns[n].Title;
		...
		public Sport.UI.Controls.Style GetStyle(int row, int field, Sport.UI.Controls.GridDrawState state)
		{
			...
			if (this.Championship is Sport.Championships.CompetitionChampionship)
			{
				...
				{
					Sport.Championships.Team team = (Sport.Championships.Team) _rows[row];
					string strTitle = _grid.Groups[1].Columns[field].Title;
					Sport.Common.EquationVariables var = new Sport.Common.EquationVariables();
					team.SetFields(var);
					string strText = _rankingTable.Fields[field - 2].Evaluate(var);
					if (strTitle.IndexOf("נוספות") < 0 && strTitle != "נקודות" && strText != "???")
					{
						if (Sport.Common.Tools.IsInteger(strText) && Int32.Parse(strText) <= 0)
						{
