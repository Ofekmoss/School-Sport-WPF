...
namespace Sportsman.Producer
{
		...
		Sport.UI.Controls.IGridSource
	{
		...
		public Sport.UI.Controls.Style GetStyle(int row, int field, Sport.UI.Controls.GridDrawState state)
		{
			...
			if (_rows[row] is Sport.Championships.Team)
			{
				...
				if (team is Sport.Championships.CompetitionTeam)
				{
					...
					{
						bool blnHaveToCheck = true;
						if (this.Group != null && this.Group is Sport.Championships.CompetitionGroup)
						{
							Sport.Championships.CompetitionGroup compGroup = (Sport.Championships.CompetitionGroup)this.Group;
							if (_competitionIndex >= 0 && _competitionIndex < compGroup.Competitions.Count)
								blnHaveToCheck = !compGroup.Competitions[_competitionIndex].IsRelayRace();
						}
		...
		public Sport.Championships.Team[] GetSelectedTeams()
		{
			System.Collections.ArrayList arrSelectedTeams = new System.Collections.ArrayList();
			if (_grid.Selection.Size > 0)
			{
				foreach (int rowIndex in _grid.Selection.Rows)
				{
					if (_rows[rowIndex] is Sport.Championships.Team)
					{
						arrSelectedTeams.Add((Sport.Championships.Team)_rows[rowIndex]);
					}
				}
			}
			return (Sport.Championships.Team[])arrSelectedTeams.ToArray(typeof(Sport.Championships.Team));
		}
	}
