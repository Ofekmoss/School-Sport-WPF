...
namespace Sportsman.Producer
{
	public class PlayerCardItem : PageItem
	{
		...
		static PlayerCardItem()
		{
			...
			string filePath = Mir.Common.Utils.Instance.MapPath("PlayerCardSettings.txt");
			//System.Windows.Forms.MessageBox.Show("File '" + filePath + "' exists? " + System.IO.File.Exists(filePath), "Debug",
			//	System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information, System.Windows.Forms.MessageBoxDefaultButton.Button1);
			if (!System.IO.File.Exists(filePath))
			{
				//System.Windows.Forms.MessageBox.Show("Files in directory: '" + string.Join(", ", System.IO.Directory.GetFiles(System.IO.Path.GetDirectoryName(filePath)).ToList().ConvertAll(f =>
				//	System.IO.Path.GetFileName(f))), "Debug", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information, System.Windows.Forms.MessageBoxDefaultButton.Button1);
			}
			definitions = Core.Tools.ParseDefinitionFile(filePath);
			//System.Windows.Forms.MessageBox.Show("Definitions count: " + definitions.Count, "Debug",
			//	System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information, System.Windows.Forms.MessageBoxDefaultButton.Button1);
			if (definitions.Count > 0)
			{
				//System.Windows.Forms.MessageBox.Show("Values: " + string.Join(", ", definitions.Keys.ToList().ConvertAll(k => string.Format("{0}={1}", k, definitions[k]))), "Debug",
				//	System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information, System.Windows.Forms.MessageBoxDefaultButton.Button1);
			}
		}
		...
		public override void PaintItem(Graphics g)
		{
			...
			int labelHeight = Sport.Core.Configuration.ReadInt("GeneralSettings", "PlayerCardLineHeight", 15);		//height of label
			...
			int labelVerticalGap = Sport.Core.Configuration.ReadInt("GeneralSettings", "PlayerCardVerticalGap", 7); //vertical gap between each label
			...
			//int pictureOffsetLeft = Core.Tools.GetValueOrDefault(definitions, "OffsetLeft", 0);
			//int pictureOffsetTop = Core.Tools.GetValueOrDefault(definitions, "OffsetTop", 0);
			//int pictureOverrideWidth = Core.Tools.GetValueOrDefault(definitions, "PictureWidth", 0);
			//int pictureOverrideHeight = Core.Tools.GetValueOrDefault(definitions, "PictureHeight", 0);

			int forcedTopMargin = Sport.Core.Configuration.ReadInt("GeneralSettings", "PlayerCardTopMargin", 0);
			int left = Bounds.Left;
			int top = Bounds.Top + forcedTopMargin;

			List<KeyValuePair<string, string>> dataItems = new List<KeyValuePair<string, string>>();
			dataItems.Add(new KeyValuePair<string, string>("שם הענף: ", _player.Team.Championship.Sport.Name));
			dataItems.Add(new KeyValuePair<string, string>("שם הספורטאי/ת: ", _player.Name));
			dataItems.Add(new KeyValuePair<string, string>("ת.ז: ", _player.Student.IdNumber));
			dataItems.Add(new KeyValuePair<string, string>("תאריך לידה: ", _player.Student.BirthDate.ToString("dd/MM/yyyy")));
			dataItems.Add(new KeyValuePair<string, string>("כיתה: ", new Sport.Types.GradeTypeLookup(true).Lookup(_player.Student.Grade)));
			dataItems.Add(new KeyValuePair<string, string>("שם בית הספר: ", _player.Student.School.Name));
			dataItems.Add(new KeyValuePair<string, string>("שנת לימודים: ", Sport.Common.Tools.GetHebrewYear(_player.Team.Championship.Season.Id)));
			dataItems.Add(new KeyValuePair<string, string>("חריג גיל: ", _player.IsOverAge() ? "כן" : "לא"));
			List<int> captionWidths = dataItems.ConvertAll(dataItem =>
			{
				return (int)(g.MeasureString(dataItem.Key, labelFont, Bounds.Width).Width + 0.5);
			});
			int maxCaptionWidth = captionWidths.Max();
			int textWidth = Bounds.Width - maxCaptionWidth;
			dataItems.ForEach(dataItem =>
			{
				string caption = dataItem.Key;
				string text = dataItem.Value;
				g.DrawString(caption, labelFont, ForeBrush, new Rectangle(left, top, Bounds.Width, labelHeight), nameFormat);
				g.DrawString(text, labelFont, ForeBrush, new Rectangle(left, top, textWidth, labelHeight), nameFormat);
				top += labelHeight + labelVerticalGap;
			});
			if (_picture != null)
			{
				Size picSize = Sport.Common.Tools.SmartResize(_picture, Math.Max(pictureWidth, pictureHeight));
				try
				{
					g.DrawImage(_picture, Bounds.Left + 5, Bounds.Top + 10 + forcedTopMargin, picSize.Width, picSize.Height);
				}
				catch
				{
				}
			...
			{
				Size picSize;
				if (pictureOverrideWidth > 0 && pictureOverrideHeight > 0)
					picSize = new Size(pictureOverrideWidth, pictureOverrideHeight);
				else
					picSize = Sport.Common.Tools.SmartResize(_picture, Math.Max(pictureWidth, pictureHeight));
				g.DrawImage(_picture, left + pictureOffsetLeft,
					top + 10 + pictureOffsetTop, picSize.Width, picSize.Height);
			}
	...
	public class PlayerCardsObject2 : PageItem, ISectionObject
	{
		...
		public bool SavePage(DocumentBuilder builder, Page page)
		{
			...
			int rightMargin = Sport.Common.Tools.CIntDef(
				Sport.Core.Configuration.ReadString("PlayerCardMargins",
				"SideMargin"), 0);
			int labelWidth = 200;		//maximum width of the labels.
			int seperatorWidth = 20;	//width of seperator between label and caption
			int captionWidth = 80;		//maximum width of label caption.
