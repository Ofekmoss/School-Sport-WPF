...
namespace Sportsman.Producer
{
	...
	public class RulesetEditorView : Sport.UI.View
	{
		...
		private void SaveClicked(object sender, EventArgs e)
		{
			...
			try
			{
				blnSuccess = _ruleset.Save();
			}
			catch (Exception ex)
			{
				Sport.UI.MessageBox.Error("אירעה שגיאה בעת שמירת הנתונים אנא נסה שנית עוד מספר שניות", "שמירת נתונים");
	...
	public class RulesetGridSource : Sport.UI.Controls.IGridSource
	{
		...
		public void Remove(int row)
		{
			if (rows != null)
			{
				...
				{
					Row r = rows[row];
					if (r != null && _ruleset[r.RuleType] != null)
