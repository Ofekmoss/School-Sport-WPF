...
namespace Sport.UI
{
	public class PrintControllerWithPageForm : PrintController
	{
		...
		public override Graphics OnStartPage(PrintDocument document, PrintPageEventArgs e)
		{
			...
			{
				Sport.UI.MessageBox.Error("לא ניתן להדפיס, אנא נסה שנית מאוחר יותר\nפרטי שגיאה:\n" + ex.Message, "הדפסה");
				e.Cancel = true;
				return null;
			}
			...
			try
			{
				graphics = this.underlyingController.OnStartPage(document, e);
			}
			catch (Exception ex)
			{
				Sport.UI.MessageBox.Error("לא ניתן להדפיס, אנא נסה שנית מאוחר יותר\nפרטי שגיאה:\n" + ex.Message, "הדפסה");
		...
		public override void OnStartPrint(PrintDocument document, PrintEventArgs e)
		{
			...
			catch (Exception exception)
			{
				...
				if (exception.Message.IndexOf("שם המדפסת אינו חוקי") >= 0)
				{
					Sport.UI.MessageBox.Error("שם מדפסת לא חוקי אנא בחר מדפסת אחרת או שנה שם", "הדפסה");
				}
				else
				{
					if (exception.Message.IndexOf("The operation completed successfully") < 0)
					{
						Sport.UI.MessageBox.Error("שגיאה כללית בעת הדפסה אנא נסה שנית מאוחר יותר:\n" + exception.Message, "הדפסה");
					}
				}
			}
			finally
