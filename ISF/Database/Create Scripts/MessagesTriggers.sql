-- ================================================================================
--Championship Status Changed - 
--Sending messages to the proper supervisors when championship status is changed to
--Team or Player Register mode.
-- ================================================================================
DROP TRIGGER T_CHAMPIONSHIP_STATUS_CHANGED
GO

CREATE TRIGGER T_CHAMPIONSHIP_STATUS_CHANGED
ON CHAMPIONSHIPS
AFTER UPDATE AS
BEGIN
  ...
  BEGIN
    ...
    BEGIN
		...
		BEGIN
			INSERT INTO MESSAGES(USER_ID, MESSAGE_TYPE, MESSAGE_STATUS, MESSAGE_TEXT, 
								TIME_SENT)
			VALUES(@user, 1, 1, 'האליפות "'+@name+'" נפתחה לרישום קבוצות', GETDATE())
    ...
    BEGIN
		...
		BEGIN
			INSERT INTO MESSAGES(USER_ID, MESSAGE_TYPE, MESSAGE_STATUS, MESSAGE_TEXT, 
								TIME_SENT)
			VALUES(@user, 2, 1, 'האליפות "'+@name+'" נפתחה לרישום שחקנים', GETDATE())
