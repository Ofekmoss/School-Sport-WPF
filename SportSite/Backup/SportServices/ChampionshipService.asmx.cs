...
namespace SportServices
{
	...
	public class ChampionshipService : System.Web.Services.WebService
	{
		...
		{
			string strSQL = "SELECT PLAYER_NUMBER, HEAT, POSITION, RESULT_POSITION, " +
				"RESULT, SCORE, CUSTOM_POSITION " +
			...
			SqlCommand command)
		{
			command.CommandText = "IF NOT EXISTS(SELECT * FROM CHAMPIONSHIP_COMPETITION_COMPETITORS WHERE CHAMPIONSHIP_CATEGORY_ID=@0 AND PHASE=@1 AND NGROUP=@2 AND COMPETITION=@3 AND PLAYER_NUMBER=@5) " +
				...
				"CHAMPIONSHIP_CATEGORY_ID, PHASE, NGROUP, COMPETITION, " +
				"COMPETITOR, PLAYER_NUMBER, HEAT, POSITION, RESULT_POSITION, " +
				"RESULT, SCORE, CUSTOM_POSITION) VALUES " +
			...
			for (int n = 0; n < competitors.Length; n++)
			{
				...
				command.Parameters["@5"].Value = competitors[n].PlayerNumber;
				...
				if (competitors[n].Heat >= 0)
					command.Parameters["@6"].Value = competitors[n].Heat;
				if (competitors[n].Position >= 0)
					command.Parameters["@7"].Value = competitors[n].Position;
				if (competitors[n].ResultPosition >= 0)
					command.Parameters["@8"].Value = competitors[n].ResultPosition;
				if (competitors[n].Result >= 0)
					command.Parameters["@9"].Value = competitors[n].Result;
				if (competitors[n].Score >= 0)
					command.Parameters["@10"].Value = competitors[n].Score;
				if (competitors[n].CustomPosition >= 0)
					command.Parameters["@11"].Value = competitors[n].CustomPosition;
				try
				{
