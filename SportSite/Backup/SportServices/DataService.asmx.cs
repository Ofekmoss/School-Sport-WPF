...
namespace SportServices
{
	...
	public class DataService : System.Web.Services.WebService
	{
		...
		{
			string strSQL = "Select t.TEAM_ID, t.CHAMPIONSHIP_ID, t.CHAMPIONSHIP_CATEGORY_ID, Count(p.PLAYER_ID) As PlayersCount " +
				"From Teams t Left Join Players p On p.TEAM_ID=t.TEAM_ID " +
				"Inner Join CHAMPIONSHIPS c On t.CHAMPIONSHIP_ID=c.CHAMPIONSHIP_ID " +
				"Where c.SEASON=@season And t.DATE_DELETED Is Null And p.DATE_DELETED Is Null " +
				"Group By t.TEAM_ID, t.CHAMPIONSHIP_ID, t.CHAMPIONSHIP_CATEGORY_ID Having Count(p.PLAYER_ID)>0 " +
				"Order By PlayersCount Desc";
			List<SimpleData> playerMapping = new List<SimpleData>();
			SimpleTable table = DB.Instance.GetDataBySQL(strSQL, new SimpleParameter("@season", season));
			table.Rows.ForEach(row => playerMapping.Add(new SimpleData
			{
				ID = (int)row[0], 
				Name = string.Format("{0},{1},{2}", row[1], row[2], row[3])
			}));
		...
		public SimplePlayerData[] GetSimplePlayerData(int[] championships)
		{
			...
			//build select statement:
			string strSQL = "SELECT t.CHAMPIONSHIP_ID, t.TEAM_ID, p.PLAYER_ID, st.FIRST_NAME, st.LAST_NAME, st.BIRTH_DATE, st.ID_NUMBER, cat.CATEGORY, sc.SCHOOL_NAME, cit.CITY_NAME " +
				"FROM ((((PLAYERS p INNER JOIN STUDENTS st ON p.STUDENT_ID=st.STUDENT_ID) " +
				"INNER JOIN TEAMS t ON p.TEAM_ID=t.TEAM_ID) " +
