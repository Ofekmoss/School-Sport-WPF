...
namespace SportServices
{
	...
	public class Common
	{
		...
		public static bool VerifySuperUser(object rawUserId)
		{
			if (rawUserId != null)
			{
				int numericUserId;
				if (Int32.TryParse(rawUserId.ToString(), out numericUserId))
				{
					string rawSuperUsers = ConfigurationManager.AppSettings["SuperUsers"];
					if (rawSuperUsers != null && rawSuperUsers.Length > 0)
						return Array.IndexOf<string>(rawSuperUsers.Split(','), numericUserId.ToString()) >= 0;
				}
			}
			return false;
		}
