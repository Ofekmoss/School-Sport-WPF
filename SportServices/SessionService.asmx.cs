...
namespace SportServices
{
	...
	public class SessionService : System.Web.Services.WebService
	{
		...
		public UserData Login(string username, string password, int season)
		{
			...
			result = _VerifyUser(strUserName, password, 1);
		...
		{
			return _VerifyUser(username, password, 100);
		}
		...
		public string GetUserPassword(string username, string askerLogin, string askerPassword)
		{
			...
			UserData user = _VerifyUser(askerLogin, askerPassword, 200);
		...
		public void ChangeUserPassword(string username, string oldPassword, string newPassword)
		{
			...
			//super user?
			//if (!Common.VerifySuperUser(Session[SessionKey_UserID]))
			//	throw new Exception("User " + Session[SessionKey_UserID] + " can't change password");
		...
		/// </summary>
		private UserData _VerifyUser(string username, string password, int source)
		{
			UserData result = new UserData();
			...
			{
				strSQL = "Insert Into FailedUserLogins (Id, [Source], Username, UserPassword) " +
						"(" +
						"	Select IsNull(Max(Id), 0)+1, @source, @name, @password From FailedUserLogins" + 
						")";
				DB.Instance.Execute(strSQL,
					new SimpleParameter("@name", username),
					new SimpleParameter("@password", password),
					new SimpleParameter("@source", source));
			}
